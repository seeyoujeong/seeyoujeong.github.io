...
변수를 선언하면 기본적으로 불변이며, 값을 변경하려면 `mut` 키워드를 사용해 가변 변수로 만들어야 한다. JS에 비유하면, 기본 변수는 `const`, `mut`는 `let`과 비슷하다.  
...
변수를 사용하면 매칭된 값을 바인딩해서 사용할 수 있다.
...
`if`를 사용해 같은 패턴이라도 조건에 따라 다르게 처리할 수 있다.
...
구조체, 열거형, 튜플을 분해하여 값의 부분들을 쓰기 위해 패턴을 사용할 수 있다.
...
`..`로 특정 부분만 사용하고 나머지는 무시하거나 `@`을 사용해 나머지를 변수에 바인딩할 수 있다.
